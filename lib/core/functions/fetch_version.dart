...
Future<AppVersionData> fetchVersion({String? playStoreId, String? appleId, String? country}) async {
  ...
  if (Platform.isAndroid) {
    data = await fetchAndroid(packageInfo: packageInfo, playStoreId: playStoreId, country: country);
  } else if (Platform.isIOS) {
    data = await fetchIOS(packageInfo: packageInfo, appleId: appleId, country: country);
  } else {
...
Future<AppVersionData> fetchAndroid({PackageInfo? packageInfo, String? playStoreId, String? country}) async {
  ...
  if (response.statusCode == 200) {
    final versionMatch = RegExp(r',\[\[\["([0-9,\.]*)"]],').firstMatch(response.body);
  ...
  if (response.statusCode == 200) {
    ...
    if (results.isEmpty) {
      ...
      return AppVersionData(
          ...
          storeUrl: results.first['trackViewUrl'],
          localVersion: packageInfo?.version,
